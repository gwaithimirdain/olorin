...
module NameIndices = struct
  ...
  type 'a suc = 'a
  type 'a scope = unit
  type 'a embed = Parser.Notation.wrapped_parse
end
...
module Resolver = struct
  ...
  let embed : ('a1, 'a2) scope -> 'a1 I1.embed -> ('a1 T1.check, 'a2 T2.check) Either.t =
   fun ctx (Notation.Wrap tm) ->
    Reporter.try_with ~fatal:(fun d -> Either.Right (Raw.Indexed.Synth (Fail d.message)))
    @@ fun () ->
    let tm = Unresolve.check ctx (Postprocess.process (Bwv.map (fun (x : name) -> x.name) ctx) tm) in
    Left tm.value
end
...
let start (parameters : Variable.js Js.t Js.js_array Js.t)
  ...
  let params =
    Array.fold_right
      (fun x xs -> Variable.of_js new_vars x :: xs)
  ...
  try
    ...
    let params =
      List.map
        (fun ({ name; ty; id = Id id; _ } : Variable.t) : Parameter.t ->
          let title =
            match name with
          ...
          {
            wslparen = [];
            (* We save the ID and has_value of each parameter node as "comments" next to its parameter name. *)
            names = [ (name, [ `Line id ]) ];
            wscolon = [];
        ...
        (fun [ x; y ] : name ->
          {
            ...
            port =
              (match y with
              | [ `Line id ] -> Some ({ vertex = Id.Id id; sort = Output; label = None } : Port.t)
