...
(* Raw.App now takes a *check* function and an *optional* check argument.  These helpers build the
   pieces from the synth/check terms we already have. *)
let named_arg (c : unit Named.check located) : unit Named.check option located =
  locate_opt c.loc (Some c.value)

let named_synth (s : unit Named.synth located) : unit Named.check located =
  locate_opt s.loc (Named.Synth s.value)
...
module Unresolver = struct
  ...
  (* Required by the Resolver interface, but only used for the Weaken term form, which we never
     generate; here the scope is a Bwv indexed by the (suc) source index, so it's always nonempty. *)
...
module Scoping = struct
  (* `bail_out` is the choice described above.  `ill_scoped` is where to record the ports the
     resolution couldn't find in scope -- a wire reaching into somewhere the value it carries
     doesn't exist, such as a box under a binder feeding one outside it.  Each caller supplies its
     own list to collect into: synth_output_port intersects one attempt's with the next, so that
     what every scope agreed was out of scope can be reported with the failure, and the fragment
     retried with those wires cut (which keeps the rest of it labeled) rather than the whole thing
     going blank with an error that names no wire at all. *)
  type t = { bail_out : bool; ill_scoped : Port.t list ref }
end
...
let note_ill_scoped (ill_scoped : Port.t list ref) (x : index) =
  match x with
  | `Port p -> if not (List.mem p !ill_scoped) then ill_scoped := p :: !ill_scoped
  | `Name _ -> ()
...
module Resolver = struct
  ...
  (* Required by the Resolver interface, but only used for the Weaken term form, which we never
     generate, so the empty case is unreachable. *)
  type (_, _) pop = Pop : ('a1, 'a2) scope * ('a2 I2.suc, 'a2s) Eq.t -> ('a1, 'a2s) pop
  ...
  let embed : ('a1, 'a2) scope -> 'a1 I1.embed -> ('a1 T1.check, 'a2 T2.check) Either.t =
   fun ctx (variables, Notation.Wrap tm) ->
    let ({ bail_out; _ } : Scoping.t) = RequireScoping.read () in
    (* The variables wired into an expr block are carried by wires just like any other, so each has
       to be in scope here, and one that isn't is an ill-scoped connection like any other.  When
       we're searching for a scope, that sends the search on to the next one (reindex raises it),
       rather than stopping here because the expression, which names them, doesn't resolve. *)
    let out_of_scope =
      match variables with
      | Wired ports ->
          List.find_map
            (fun p ->
              match reindex ctx (`Port p) with
              | Ok _ -> None
              | Error e -> Some e)
            (PortSet.elements ports)
      | All -> None in
    match out_of_scope with
    | Some e -> Either.Right (Raw.Indexed.Synth (Fail e))
    | None ->
        Reporter.try_with ~fatal:(fun d ->
            match (variables, d.message) with
            (* In an expr block, a name that doesn't resolve is always to be fixed by wiring in the
               variable it means, whether or not there is one in scope. *)
            | Wired _, Unbound_variable (x, _) ->
                Either.Right (Raw.Indexed.Synth (Fail Explain.Extern.(code (Unwired_variable x))))
            (* Anywhere else, the name might be in some other scope, so if we're searching for one
               this one is no good; the search reports it if no scope has it. *)
            | All, Unbound_variable _ when bail_out -> fatal_diagnostic d
            | _, message -> Either.Right (Raw.Indexed.Synth (Fail message)))
        @@ fun () ->
          ...
          | Wired variables ->
              Bwv.map
                (fun (x : name) ->
                  ...
                  | None -> x.name
                  | Some p ->
                      if PortSet.mem p variables || PortSet.mem p !parameter_ports then x.name
                      else None)
                ctx
              ...
              ( named_synth
                  (locate_opt None
                     (Named.ImplicitSApp
                        ( locate_opt None
                            (Named.Const
                               (Scope.lookup [ implicit_pre ] <||> "implicit_pre not found")),
                          None,
                          fn ))),
                named_arg arg,
                locate_opt None `Explicit ) in
...
(* The diagnostics in an array, each only once.  Every scope a fragment is tried in can report the
   very same thing -- a name that none of them has, say -- and it's only worth saying once. *)
let distinct_diagnostics (ds : Diagnostic.js Js.t Dynarray.t) : Diagnostic.js Js.t Dynarray.t =
  let seen = Hashtbl.create 5 in
  let distinct = Dynarray.create () in
  Dynarray.iter
    (fun (d : Diagnostic.js Js.t) ->
      let key =
        (Js.to_string d##.code, Js.to_string d##.text, Js.to_string (Js._JSON##stringify d##.locs))
      in
      if not (Hashtbl.mem seen key) then (
        Hashtbl.add seen key ();
        Dynarray.add_last distinct d))
    ds;
  distinct
...
(* On failure this reports, along with the diagnostics, the ports that were out of scope in every
   scope it tried -- the ones a caller could cut to make the rest of the fragment work. *)
let synth_output_port (run : (unit -> unit) -> unit) (vertices : Vertex.t IdMap.t)
    ...
    [ `Found_scope of Diagnostic.js Js.t Dynarray.t
    | `No_scope of Diagnostic.js Js.t Dynarray.t * Port.t list ] =
  ...
  let record_attempt (failed : Port.t list) =
    always_ill_scoped :=
      Some
        (match !always_ill_scoped with
        | None -> failed
        | Some common -> List.filter (fun p -> List.mem p failed) common) in
  let tm, _ =
  ...
  if !cyclic then `No_scope (distinct_diagnostics scoping_diagnostics, [])
  else
    ...
    let rec look_for_scope contexts =
      ...
      | [] ->
          ...
          `No_scope
            (distinct_diagnostics scoping_diagnostics, Option.value !always_ill_scoped ~default:[])
      | Context
          (type a b s)
          ((status, ctx, scope) :
...
(* Remove from a backwards graph every edge coming out of one of the given ports, returning the
   edges removed along with the pruned graph. *)
let cut_edges (bad : Port.t list) (bwd : bwd_graph) : Edge.t list * bwd_graph =
  let cut = ref [] in
  let pruned =
    TargetMap.filter_map
      (fun _ es ->
        let keep, drop = List.partition (fun (e : Edge.t) -> not (List.mem e.source bad)) es in
        cut := !cut @ drop;
        match keep with
        | [] -> None
        | keep -> Some keep)
      ...
      (fun (p, _, _) ->
        ...
        if Hashtbl.mem labels (`Port p) then None
          ...
          | `Found_scope ds ->
              ...
              None
          (* If we failed to find a working scope/context, save that port to try again later, along with the diagnostics it produced this time and whatever it found to be out of scope. *)
          | `No_scope (ds, bad) -> Some (p, ds, bad))
      ports in
...
(* A last pass over the ports no scope worked for.  If the reason was a wire reaching out of scope
   ...
   themselves, so the player can see which ones, rather than an error with no location at all) and
   synthesize the fragment again without them.  This runs only after the ordinary passes have
   finished, so it can never pre-empt a port that another pass would have done in full. *)
let synth_cut_ports (run : (unit -> unit) -> unit) (vertices : Vertex.t IdMap.t)
    (fwd_graph : fwd_graph) (bwd_graph : bwd_graph) (contexts : context list ref)
    (ports : (Port.t * Diagnostic.js Js.t Dynarray.t * Port.t list) list)
    (diagnostics : Diagnostic.js Js.t Dynarray.t) =
  (* A wire that several ports trip over is only worth reporting once. *)
  let reported : (Id.t, unit) Hashtbl.t = Hashtbl.create 5 in
  List.filter_map
    (fun (p, ds, bad) ->
      (* The passes above already tried this port in every scope, and said what they found to be out
         of scope; all we do is cut those wires and try that once. *)
      match cut_edges bad bwd_graph with
      | [], _ -> Some (p, ds)
      | cut, pruned -> (
          match synth_output_port run vertices fwd_graph pruned !contexts p with
